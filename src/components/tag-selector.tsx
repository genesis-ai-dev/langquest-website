...
export function TagSelector({
  ...
  useEffect(() => {
    ...
    async function fetchRecoveredTags() {
      ...
      data?.forEach((tag) => {
        showingSelectedTags.current.set(tag.id, tag.name);
      });
    }
  ...
  async function createNewTag(tagName: string) {
    ...
    if (
      filteredTags.find((t) => t.name.toLowerCase() === tagName.toLowerCase())
    )
      return;
    ...
    if (error) throw error;
    showingSelectedTags.current.set(data.id, data.name);
