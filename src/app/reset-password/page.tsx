...
function ResetPasswordForm() {
  ...
  const handlePasswordReset = useCallback(async () => {
    ...
    const access_token = searchParams.get('access_token') || window.location.hash.match(/access_token=([^&]*)/)?.[1]
    const refresh_token = searchParams.get('refresh_token') || window.location.hash.match(/refresh_token=([^&]*)/)?.[1]
    ...
    if (isMobile()) {
      ...
      try {
        ...
        setMessage('Error: Invalid or expired reset link. Please request a new password reset.')
        setError('red')
      }
    }
  ...
  const submitNewPassword = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setMessage('Password successfully updated! You can now close this window and log in to the app.')
      setShowForm(false)
    } catch (error: unknown) {
  ...
  return (
    <div className="container mx-auto px-4">
      {message && (
        <div id="message" className={`text-center my-5 ${error ? 'text-red-500' : ''}`}>
          {message}
        </div>
      )}
